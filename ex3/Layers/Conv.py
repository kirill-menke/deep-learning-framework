...
class Conv(Base):
    ...
    def forward(self, input_tensor):
        ...
        for k, kernel in enumerate(self.weights):
            feature_map[k] = signal.correlate(flat_input_tensor, kernel, mode='same')[slicing]
            feature_map[k] += self.bias[k]
        
    
        return feature_map.swapaxes(0, 1)
    ...
    def backward(self, error_tensor):
        ...
        upsampled_error_tensor = np.zeros((*error_tensor.shape[:2], *self.input_slice_shape))
        ...
        next_error_tensor = np.empty((len(kernels), len(error_tensor), *self.input_slice_shape))
